...
## 10. Camera Animation (Hero Section)

### Overview
Hero section pada landing page menampilkan animasi kamera 3D/rotasi untuk memberikan kesan premium dan interaktif. Animasi ini menggunakan **Framer Motion** + **CSS 3D Transform**.

### Animation Types

| Tipe | Teknologi | Kesan | Kegunaan |
|------|-----------|-------|----------|
| **3D Rotate** | Framer Motion + CSS `rotateY` | Premium, interaktif | Hero utama |
| **Float** | Framer Motion `y` animate | Elegant, dynamic | Camera card hover |
| **Parallax** | Framer Motion + scroll | Depth, engaging | Scroll effect |
| **Spin** | CSS `@keyframes` | Playful, eye-catching | Loading states |

### Implementation (Framer Motion + CSS 3D)

```tsx
// Hero Camera Animation - 3D Rotate
"use client"
...
export function CameraCard({ camera }) {
  return (
    <motion.div
      className="border border-surface-light p-4"
      whileHover={{
        y: -8,
    ...
    >
      ...
      />
      <h3 className="text-text-dominant font-display text-lg mt-4">
        {camera.name}
      </h3>
      <p className="text-text-tertiary text-sm">
        Rp {camera.price_per_day}/hari
      </p>
    </motion.div>
  )
}
...
### CSS 3D Transform (Tanpa Framer Motion)
...
.camera-rotate {
  animation: cameraRotate 20s linear infinite;
...
### Integration with Apple Design

| Elemen | Apple Style | Camera Animation |
|--------|-------------|------------------|
| **Background** | White (`#FFFFFF`) | Gradient subtle dari white |
| **Shadow** | Minimal, subtle | Glow effect blur |
| **Spacing** | 48px padding | 500px height hero |
| **Typography** | SF Pro Display 40px | Headline di sebelah animasi |
| **Color** | `#0071E3` accent | Blue glow di belakang kamera |
| **Border Radius** | 0px (sharp) | Camera tanpa border-radius |

### Animation Principles

1. **Subtle > Dramatic** - Animasi halus, tidak berlebihan
2. **Purposeful** - Setiap animasi punya makna (highlight product)
3. **Performance** - Gunakan `transform` dan `opacity` untuk GPU acceleration
4. **Responsive** - Kurangi animasi di mobile untuk performa
5. **Accessible** - Sediakan `prefers-reduced-motion` option

### Reduced Motion Support

```tsx
// Framer Motion dengan reduced motion support
"use client"
...
|--------|------|----------|
| **Spline.design** | 3D Model | Kamera 3D interaktif (gratis) |
| **Lottie Files** | JSON Animation | Animasi ringan (gratis) |
| **Dribbble** | GIF | Kamera rotasi (cari gratis) |
| **Custom CSS** | 3D Transform | Rotasi sederhana |

### Fallback (Jika Tidak Ada Asset 3D)

```tsx
// Fallback: Static image dengan subtle animation
export function HeroCameraFallback() {
  return (
    <motion.div
      className="relative"
      animate={{
